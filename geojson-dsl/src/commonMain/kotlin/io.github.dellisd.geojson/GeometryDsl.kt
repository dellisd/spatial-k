...
private val Position.lngLat get() = LngLat(longitude, latitude, altitude)
...
inline fun point(
    ...
    PointDsl(LngLat(longitude, latitude, altitude)).apply(block).create()

class MultiPointDsl(private val coordinates: MutableList<Position> = mutableListOf()) : GeometryDsl<MultiPoint>() {
    override fun create(): MultiPoint = MultiPoint(coordinates, bbox)
...
inline fun multiPoint(block: MultiPointDsl.() -> Unit): MultiPoint = MultiPointDsl().apply(block).create()

class LineStringDsl(internal val coordinates: MutableList<Position> = mutableListOf()) : GeometryDsl<LineString>() {
    override fun create(): LineString = LineString(coordinates, bbox)
...
inline fun lineString(block: LineStringDsl.() -> Unit) = LineStringDsl().apply(block).create()
...
class MultiLineStringDsl(private val coordinates: MutableList<List<Position>> = mutableListOf()) :
    ...
    inline fun lineString(block: LineStringDsl.() -> Unit): LineStringDsl = LineStringDsl().apply(block)
...
inline fun multiLineString(block: MultiLineStringDsl.() -> Unit) = MultiLineStringDsl().apply(block).create()
...
inline fun polygon(block: PolygonDsl.() -> Unit) = PolygonDsl().apply(block).create()
...
class MultiPolygonDsl(private val coordinates: MutableList<List<List<Position>>> = mutableListOf()) :
    ...
    inline fun polygon(block: PolygonDsl.() -> Unit) = PolygonDsl().apply(block)
...
inline fun multiPolygon(block: MultiPolygonDsl.() -> Unit) = MultiPolygonDsl().apply(block).create()
...
inline fun geometryCollection(block: GeometryCollectionDsl.() -> Unit) = GeometryCollectionDsl().apply(block).create()
