...
outside ±180° are accepted and round-trip through serialization unchanged. `Position.wrapped()`
normalizes longitude into `[-180, 180)` while leaving latitude and altitude unchanged.
...
`BoundingBox.wrapped()` normalizes the west longitude into `[-180, 180)` and the east longitude
into `(-180, 180]` without reordering them.
...
360° returns one full-world box from -180° to 180°. Wrapping the endpoints before splitting
discards the original span.
